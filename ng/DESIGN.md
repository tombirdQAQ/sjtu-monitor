...
- 界面：与 Windows 11“设置”应用一致的原生风格：Mica 窗口背景、系统卡片画刷（`CardBorder`、`CardButton`，见 `App.xaml`）、默认按钮；不做 macOS 式彩色玻璃。托盘右键菜单是 Win32 弹出菜单，深色模式靠 uxtheme `SetPreferredAppMode`（`NativeTheme`）。
- 已知坑：
